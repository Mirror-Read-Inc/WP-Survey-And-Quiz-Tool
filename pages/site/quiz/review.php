		...
		<?php
			if (!isset($section['questions'])){
					...
					<p class="answer_given">
						<ol>
							<?php foreach ($questionArray['answers'] as $answerKey => $answer){ ?>
								  <li><font color="<?php echo ( $answer['correct'] != 'yes' ) ?  (isset($section['answers'][$questionId]['given']) &&  in_arraylike($answerKey, $section['answers'][$questionId]['given']) ) ? '#FF0000' :  '#000000' : '#00FF00' ; ?>"><?php echo esc_html(stripslashes($answer['text'])); ?></font><?php if (isset($section['answers'][$questionId]['given']) && in_arraylike($answerKey, $section['answers'][$questionId]['given']) ){ ?> - Given<?php }?></li>
							<?php } ?>
						</ol>
					</p>
				...
				<?php } else { 
					...
					<p class="answer_given" style="background-color : #c0c0c0; border : 1px dashed black; padding : 5px;overflow:auto;height : 200px;"><?php if ( isset($section['answers'][$questionId]['given']) && is_arraylike($section['answers'][$questionId]['given']) ){ echo nl2br(esc_html(stripslashes(current($section['answers'][$questionId]['given'])))); } ?></p>
					<?php if (isset($questionArray['explanation'])) { 
						// replace the tokens
						$explanation = $objTokens->doReplacement( $questionArray['explanation'] );
						?>
